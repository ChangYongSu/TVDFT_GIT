...
//	int m_nNoKeyPress;
	...
	double m_fBandErrorPixelPercentage[5];
	double m_fAllowedBandErrorPixelPercentage[5];
	double m_fGoodPixelPercentage;
...
public:
	...
	void RedBlueSwap(unsigned char *pImgBufIn, int nWidth, int nHeight, int nRB_Swap);
	void HsyncModeChange(int nDeHsyncEnable, int nHyncEdgeEnable);
	void Rotate(CBaseImage& SrcImage, float fRotateFactor);
...
//	void SetNoAllowedErrorPixel(float fAllowedErrorPixel, float fAllowedErrorPixelForOsdTest);
	...
	//+add kwmoon 080715
	void SetAllowedErrorPixel(double fAllowedBandErrorPixelPercentage[]);
