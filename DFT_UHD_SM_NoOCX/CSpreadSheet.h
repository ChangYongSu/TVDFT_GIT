...
public:
	...
	bool AddRowFast(CStringArray &RowValues, long row = 0, bool replace = false); // Insert or replace a row into spreadsheet. Default is add new row. 
	bool DeleteRow(CStringArray &RowValues);
	...
	bool ReplaceRows2(CStringArray &NewRowValues); // Search and replace rows in Excel spreadsheet
	...
	inline void GetDivisions (CStringArray &Divisions) {Divisions.RemoveAll(); Divisions.Copy(m_aDivision);} // Get the header row from spreadsheet
...
private:
	...
	CStringArray m_aDivision; // Content of all the rows in spreadsheet
...
bool CSpreadSheet::DeleteRow(CStringArray &RowValues)
{
	...
	sRowString.MakeLower(); 

	for ( tempRow = 1; tempRow < m_dTotalRows; tempRow++)
	{
		m_stempString = m_aRows.GetAt(tempRow);
		m_stempString.MakeLower();
		if(sRowString == m_stempString)
		{
//			sStringTemp = m_aRows.ElementAt(tempRow); 
//			delete m_aRows.GetAt(tempRow);
			m_aRows.RemoveAt(tempRow);
			m_aRows.FreeExtra();
//			sStringTemp.FreeExtra();
...
bool CSpreadSheet::ReplaceRows(CStringArray &NewRowValues, CStringArray &OldRowValues)
{
	...
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
		m_stempSql.Format("UPDATE [%s$A1:IV65536] SET ", m_sSheetName);
...
// Search and replace rows in Excel spreadsheet
bool CSpreadSheet::ReplaceRows2(CStringArray &NewRowValues)
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
		...
		m_stempString.Format("[%s]='%s', ", m_aFieldNames.GetAt(0), NewRowValues.GetAt(0));
		m_stempSql = m_stempSql + m_stempString;

		for (int i = 3; i < NewRowValues.GetSize(); i++)
		{
			m_stempString.Format("[%s]='%s', ", m_aFieldNames.GetAt(i), NewRowValues.GetAt(i));
			m_stempSql = m_stempSql + m_stempString;
		}
		...
		for (int j = 1; j < 3; j++)
		{
			m_stempString.Format("[%s]='%s' AND ", m_aFieldNames.GetAt(j), NewRowValues.GetAt(j));
			m_stempSql = m_stempSql + m_stempString;
		}
		m_stempSql.Delete(m_stempSql.GetLength()-4, 5);
		m_stempSql += ")";
...
bool CSpreadSheet::Open()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
	   ...
	   TRY
		{
			if(!m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog))
			{
				return false;
			}
		}
		CATCH(CDBException, e)
		{
				AfxMessageBox("[ModelList.xls] File Error : " + e->m_strError);
				return false;
		}
		END_CATCH;
		...
		m_sSql.Format("SELECT * FROM [%s$A1:IV65536] ORDER BY 2, 3", m_sSheetName);
		try
		{
...
//			m_dTotalRows++; // Keep count of total number of rows
			...
			if(m_stempString != "\"F1\",;.?\"F2\",;.?\"F3\",;.?\"F4\",;.?\"F5\",;.?\"F6\"")
			{
				m_dTotalRows++; // Keep count of total number of rows
				m_aRows.Add(m_stempString);
			}
...
//				m_dTotalRows++; // Keep count of total number of rows
		...
		m_aDivision.RemoveAll(); 
		m_sSql.Format("SELECT Division FROM [%s$A1:IV65536] GROUP BY Division", m_sSheetName);
		try
		{
			m_rSheet->Open(CRecordset::forwardOnly, m_sSql, CRecordset::readOnly);
		}
		...
		if (m_dTotalColumns != 0)
		{
			...
			while (!m_rSheet->IsEOF())
			{
				try
				{
					m_rSheet->GetFieldValue((short)0, m_stempString);
