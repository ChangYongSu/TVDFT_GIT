...
//	}
	...
	//+del kwmoon 080326
//	if(nNoMeasureAudioResetRetry >= (MAX_NO_MEASURE_AUDIO_RETRY-2))
//	{
//		goto MEASURE_FREQ_START;
//	}
	...
	//+del kwmoon 080326
/*	if((AvgLevel_L < (double)MIN_AUDIO_LEVEL)
	|| (AvgLevel_L > (double)MAX_AUDIO_LEVEL)
	|| (AvgLevel_R < (double)MIN_AUDIO_LEVEL)
	|| (AvgLevel_R > (double)MAX_AUDIO_LEVEL))
	{
		if(nNoMeasureAudioLevelRetry++ < MAX_NO_MEASURE_AUDIO_RETRY)
		{
			_Wait(300); 
			szMsg.Format("Retry Measure Audio Level (%d/%d)",nNoMeasureAudioLevelRetry,(MAX_NO_MEASURE_AUDIO_RETRY));
			ShowSubStepMessage(szMsg, "Retry Measure Audio Level");
	...
	//		
	//		szMsg.Format("Reset Audio Device (%d/%d)",nNoMeasureAudioResetRetry,(MAX_NO_MEASURE_AUDIO_RETRY-2));
...
//	szLeftLevel.Format("%dmV",(int)AvgLevel_L);
//	g_pView->UpdateInfo(AUDIO_L_LEVEL,szLeftLevel);

//	szRightLevel.Format("%dmV",(int)AvgLevel_R);
...
//	&& (pAvSwitch->m_fMeasuredLeftLevel <= (pAvSwitch->m_fLeftLevelUpperLimit)))
	if(pAvSwitch->m_fMeasuredLeftLevel >= (pAvSwitch->m_fLeftLevelLowerLimit))
	{
		aResult[2] = TEST_PASS;
	}
...
//	&& (pAvSwitch->m_fMeasuredRightLevel <= (pAvSwitch->m_fRightLevelUpperLimit)))
	...
	if((aResult[2] == TEST_FAIL) || (aResult[3] == TEST_FAIL))
	{
		...
		//	_Wait(500);
			...
			{
				szMsg.Format("    Audio Level Retry(%d)",nNoMeasureAudioLevelRetry);
				AddTimeData(szMsg);
			}
...
/*	if((Avgfreq_L < (double)MIN_AUDIO_FREQ)
	|| (Avgfreq_L > (double)MAX_AUDIO_FREQ) 
	|| (Avgfreq_R < (double)MIN_AUDIO_FREQ) 
	|| (Avgfreq_R > (double)MAX_AUDIO_FREQ)) 
	{
		if(nNoMeasureAudioFreqRetry++ < MAX_NO_MEASURE_AUDIO_RETRY)
		{
			_Wait(300); 
			szMsg.Format("Retry Measure Audio Freq (%d/%d)",nNoMeasureAudioFreqRetry,(MAX_NO_MEASURE_AUDIO_RETRY));
			ShowSubStepMessage(szMsg, "Retry Measure Audio Freq");
			goto MEASURE_FREQ_START;
		}
		else
		{
...
*/
	//+change kwmoon 080523
//	szLeftFreq.Format("%dHz",(int)Avgfreq_L);
//	g_pView->UpdateInfo(AUDIO_L_FREQ,szLeftFreq);

//	szRightFreq.Format("%dHz",(int)Avgfreq_R);
//	g_pView->UpdateInfo(AUDIO_R_FREQ,szRightFreq);
	...
	if((aResult[0] == TEST_FAIL) || (aResult[1] == TEST_FAIL))
	{
		...
		//	_Wait(500); 
			...
			if(CurrentSet->bSaveProcessingTimeData)
			{
				szMsg.Format("    Audio Freq Retry(%d)",nNoMeasureAudioFreqRetry);
				AddTimeData(szMsg);
			}
...
UINT CAvSwitchBox::CheckReceiveMsg(int nCmd, int nWaitLimit, CString& sValue)
{
	...
	else 
	{
		...
		else sRst.Format("%c%c%c%c%c", m_strReceive[1], m_strReceive[2],m_strReceive[3],m_strReceive[4], m_strReceive[5]);
		sRst.MakeUpper();
		...
		else if(sRst == "DREPL") nRtn =  2;
		else if(sRst == "DBUSY") nRtn =  4;
		else nRtn = 2;
	}
	return nRtn;
}
...
BOOL CAvSwitchBox::SendRemoteCmd(int nWaitLimit, CString sCmd)
{
	// 0 : NO Test, 1 : Receive Pass, 2 : Receive Fail, 3 : Receive TimeOut, 4 : Cmd Send Retry, 5 : Send Cmd Fail
	CString szCmdString		= _T("");
	CString szSwitchBoxMsg	= _T("");
...
//	_Wait(100);
	...
	nValue		   = hexCstr2decNum(sCmd);
	szCmdString	   = MakeCmdString(SEND_REMOTE_CODE,szSwitchBoxMsg,  nValue, 0);
...
//		sTmp.Format("RemoteKey %s : %s", sCmd, m_strErrMsg);
		//-
