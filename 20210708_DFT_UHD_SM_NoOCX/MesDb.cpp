...
//			strRtn.Format(sString);
	...
	strTmp = strDsn;
	strTmp.MakeUpper();
	if(strTmp.Find("DMAMES") != -1){
		sTable = MAMES_DFT_MASTER_TABLE;
	}
	else{
		sTable = MES_DFT_MASTER_TABLE;
	}
	...
	try
	{
		...
		if(bUseLineNo)
		{
			strSql.Format("Select count(lotid) from %s where ((lotid='%s') and (line_id=%d))"
						, (const char*)sTable, (const char*)strWipid, CurrentSet->nLineNo);
		}
		else{
			strSql.Format("Select count(lotid) from %s where lotid='%s'"
						, (const char*)sTable, (const char*)strWipid);
		}
...
bool CMesDb::GetOrgId_Master_tb_Sql(CString strModelSuffix, long& nOrgId, CString &strRtn)
{
	CString			strConn, strTmp;
	ADODB::_CommandPtr		pCmd  = NULL;
	ADODB::_ParameterPtr	pPara = NULL;
...
//	CString sTable			= _T("");
	...
	strTmp = CurrentSet->strMesDsn;
	strTmp.MakeUpper();
	if(strTmp.Find("DMAMES") != -1){bMaMes = TRUE;}
...
//			strRtn.Format(sString);
	...
	if(!m_bDBconnection || m_pConn == NULL)
	{
		...
		{	
			...
			strConn = "";
			strConn = strConn + "Provider=MSDAORA;";
			strConn = strConn + "Data Source=" + CurrentSet->strMesDsn + ";";
			
			TESTHR(m_pConn->Open((_bstr_t)strConn,(_bstr_t)CurrentSet->strMesUserId,(_bstr_t)CurrentSet->strMesPassWord,NULL));
		}
	...
	try
	{
		...
		if(bMaMes){
			strSql.Format("SELECT max(organization_id) as organization_id FROM MAMES.TB_CM_ITEMMASTER WHERE  PART_NO = '%s'", strModelSuffix);
		}
		else{
			strSql.Format("SELECT max(organization_id) as organization_id FROM MES.TB_CM_ITEMMASTER WHERE  PART_NO = '%s'", strModelSuffix);
		}
		...
		vData= pRec->Fields->GetItem("organization_id")->Value; 
//		if(vData == VT_NULL)
//		{ 
//			return false;
//		}
...
bool CMesDb::GetToolOption_Master_tb_Sql(CString strModelSuffix,  long nOrgId, CString &strRtn)
{
	CString			strConn, strTmp;
	ADODB::_CommandPtr		pCmd  = NULL;
	ADODB::_ParameterPtr	pPara = NULL;
...
//			strRtn.Format(sString);
	...
	try
	{
		...
		if(bMaMes){
			strSql.Format("SELECT NVL(COMPONENT_REMARKS, ' ') AS WORK_DESC FROM TABLE(MAMES.PKG_GERP_WS.FN_BOM_BOM_EXPLOSION( %d,'%s', '', '', SYSDATE)) WHERE  UPPER(COMPONENT_REMARKS) LIKE '%%TOOL OPTION%%'", nOrgId, strModelSuffix);
		}
		else{
			strSql.Format("SELECT NVL(COMPONENT_REMARKS, ' ') AS WORK_DESC FROM TABLE(MES.PKG_GERP_WS.FN_BOM_BOM_EXPLOSION( %d,'%s', '', '', SYSDATE)) WHERE  UPPER(COMPONENT_REMARKS) LIKE '%%TOOL OPTION%%'", nOrgId, strModelSuffix);
		}
		...
		vData= pRec->Fields->GetItem("WORK_DESC")->Value; 
//		if(vData == VT_NULL)
//		{ 
//			return false;
//		}
//		else{
			sString =	vData.bstrVal;
			sString.MakeUpper();
			...
			sTemp = sBuf.Right(sBuf.GetLength() - (sBuf.Find("TOOL OPTION") + 11));
			sTemp.TrimLeft("(");
			sTemp.TrimRight(")");
			Get_ToolOption(sTemp);

	/*		sString.Delete(0, nFind +1);
			nCount = sString.Find("COUNTRY GROUP");
			if(nCount == -1){
				CurrentSet->sCountryGroup = _T("");
				CurrentSet->sCountry = _T("");
				return false;
			}
			nFind = sString.Find('/');
			sTemp = sString.Left(nFind);
			Get_CountryGroup(sTemp);

			sString.Delete(0, nFind +1);
			nCount = sString.Find("COUNTRY");
			if(nCount == -1){
				CurrentSet->sCountry = _T("");
				return true;
			}
...
CString	CMesDb::Make_Master_tb_Sql(int nType, int nSeqNo, CString strWipid, BOOL bResult, CString strUserid, float fTime)
{
	...
	int nOrganization_ID = CurrentSet->nOrganization_ID;
	...
	{
		if(nOrganization_ID == 205417){
			nLineNo = CurrentSet->nLineID_GPZ;
		}
		else if(nOrganization_ID == 205419){
			nLineNo = CurrentSet->nLineID_GMZ;
		}
		else{
			nLineNo = CurrentSet->nLineID_GLZ;
		}

		nTemp =CurrentSet->sModelSuffixName.Find('.');
		sModel = CurrentSet->sModelSuffixName.Left(nTemp);
		sSuffix =  CurrentSet->sModelSuffixName;
		sSuffix.Delete(0,nTemp + 1);

		sSqlString.Format("Insert into %s (seq, lotid, result, machine_no, line_id, time, chassis,organization_id,model,suffix) \
				values(%d,'%s', '%s', %d, %d, %.1f,'%s',%d,'%s','%s')"
				, (const char*)sTable, nSeqNo,(const char*)strWipid, (const char*)sResult, nMachineNo, nLineNo, 
				fTime,(const char*)sChassis, nOrganization_ID, sModel, sSuffix);
	}
	else{
		sSqlString.Format("Insert into %s (seq, lotid, result, machine_no, line_id, time, chassis) \
					values(%d,'%s', '%s', %d, %d, %.1f,'%s')"
					, (const char*)sTable, nSeqNo,(const char*)strWipid, (const char*)sResult, nMachineNo, nLineNo, fTime,(const char*)sChassis);
	}
...

{
	...
	strTmp.MakeUpper();
	if(strTmp.Find("DMAMES") != -1){
		sTable = MAMES_DFT_DETAIL_TABLE;
	}
	else{
		sTable = MES_DFT_DETAIL_TABLE;
	}

	if(CurrentSet->nToolOptionSetMethod == 0)
	{
		if(CurrentSet->nOrganization_ID == 205417){
			nLineNo = CurrentSet->nLineID_GPZ;
		}
		else if(CurrentSet->nOrganization_ID == 205419){
			nLineNo = CurrentSet->nLineID_GMZ;
		}
		else{
			nLineNo = CurrentSet->nLineID_GLZ;
	...
	if(nType == 0)
	{
		...
		if(pStep->m_nStepType == PROCESS_FUNCTION)
		{
			sSqlString.Format("insert into %s (seq, lotid, run, step_no, step_name, time, message, machine_no, line_id)\
							   ...
							   ,sTable, nSeqNo, strWipid, pStep->m_bTest, pStep->m_nStep
...
CString	CMesDb::Make_Dft_tb_Sql_Local(CString strWipid, CStep* pStep)
{
	...
	if(pStep->m_nStepType == PROCESS_FUNCTION)
	{
		sTmp.Format(" insert into %s (seq, lotid, run, step_no, step_name, time, message, machine_no, line_id) values (", sTable);
		sSqlString = sTmp + "%d";
	...
	else if(pStep->m_nStepType == CHECK_FUNCTION)
	{
		...
		if(pStep->m_nFuncType == MEAS_BOOL)
		{
			...
			if(pStep->m_bRunAudioTest)
			{
				sTmp.Format("insert into %s (seq, lotid, run, step_no, step_name, result, a_target, a_measure, time, message, machine_no, line_id) values (", sTable);
				sSqlString = sTmp + "%d";
			...
			else
			{
				if(pStep->m_bAdcValueCheckStep)
				{
					sTmp.Format("insert into %s (seq, lotid, run, step_no, step_name, result, a_measure, time, message, machine_no, line_id) values (", sTable);
					sSqlString = sTmp + "%d";
				...
				{
					sTmp.Format("insert into %s (seq, lotid, run, step_no, step_name, result, time, message, machine_no, line_id) values (", sTable);
					sSqlString = sTmp + "%d";
		...
		{
			sTmp.Format("insert into %s (seq, lotid, run, step_no, step_name, result, measure, target, lower_limit, upper_limit, a_target, a_measure, time, message, machine_no, line_id) values (", sTable);
			sSqlString = sTmp + "%d";
...
CString	CMesDb::Make_Master_tb_Sql_Local(CString strWipid, BOOL bResult, float fTime)
{
	...
	if(CurrentSet->nToolOptionSetMethod == 0)
	{
		...
		sTmp.Format("Insert into %s (seq, lotid, result, machine_no, line_id, time, chassis,organization_id,model,suffix) values(", sTable);
		sSqlString = sTmp + "%d";

		sTmp.Format(",'%s', '%s', %d, %d, %.1f,'%s',%d,'%s','%s')", strWipid, sResult, nMachineNo, nLineNo, fTime, sChassis, nOrganization_ID, sModel, sSuffix);
		sSqlString = sSqlString + sTmp;
	}
	else{
		sTmp.Format("Insert into %s (seq, lotid, result, machine_no, line_id, time, chassis) values(", sTable);
		sSqlString = sTmp + "%d";
