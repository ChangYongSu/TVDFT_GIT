...
void CSignalTestResultDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_STATIC_TITLE, m_szTabTitle);
	//}}AFX_DATA_MAP
}
...
BOOL CSignalTestResultDlg::OnInitDialog() 
{
	...
	m_szTabTitle = _T("No of Low/High Bits");
	//
...
void CSignalTestResultDlg::UpdateGrid()
{
	...
	for(int i = 2; i>=0; i--)
	{
		for(int j = 0; j<(NO_COL-1); j++)
		{
			...
			if(j < (m_nBitCount/3))
			{
				sNoRefHighBit.Format("%8d/%8d",m_nTotalNoMaskedPixel-nNoRefHighBit,nNoRefHighBit);
				sNoGrabHighBit.Format("%8d/%8d",m_nTotalNoMaskedPixel-nNoGrabHighBit,nNoGrabHighBit);
