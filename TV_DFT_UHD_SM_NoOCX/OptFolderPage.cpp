...
void COptFolderPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_MODELVF_FOLDER, m_strModelVFConfigFolder);
#endif
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(COptFolderPage, CPropertyPage)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BTN_OPEN_MODELVF_FOLDER, &COptFolderPage::OnBnClickedBtnOpenModelvfFolder)
END_MESSAGE_MAP()
...
#ifdef SM_MODIFY_CODE__
	m_strModelVFConfigFolder = CurrentSet ->sModelVFConfigFolder;
#endif
	UpdateData(FALSE);
...
void COptFolderPage::OnBtnOpenModelList() 
{
	...
	CFileDialog dlg(TRUE, "xls", NULL, 
		OFN_LONGNAMES | OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_NONETWORKBUTTON | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR | OFN_HIDEREADONLY, 
		"xls File(*.xls)|*.xls|", NULL);

	CString strFolder = "D:\\DFT";// CurrentSet->sModelListPath.TrimRight("\\");
...
// #ifdef SM_MODIFY_CODE__
void COptFolderPage::OnBnClickedBtnOpenModelvfFolder()
{
	...
	if (GetFolder(&strFolderPath, "Model Info  Root Foler.", this->m_hWnd, NULL, (const char*)m_strModelVFConfigFolder))
	{
		if (!strFolderPath.IsEmpty())
		{
			m_strModelVFConfigFolder = strFolderPath;
			CurrentSet->sModelVFConfigFolder = strFolderPath;
