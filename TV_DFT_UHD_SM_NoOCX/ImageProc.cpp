...
//========================
#define FULL_HD_IMAGE_SIZE	8294400	 // 2560*1080*3 = 8294400
#define UHD_IMAGE_SIZE		24883200 // 3840*2160*3 = 24883200


BYTE g_ImageBuffer1[UHD_IMAGE_SIZE];
BYTE g_ImageBuffer2[UHD_IMAGE_SIZE];
BYTE g_ImageBuffer3[UHD_IMAGE_SIZE];
...
int CImageProc::RGBTest(int nType, CBaseImage& SnapImage, CBaseImage& RefImage, CBaseImage& MaskImage,
						...
						BOOL bUseMaskImage,CProgressCtrl& Progress)
{
	int nProgress	= 0;
...
//	memset(pProcessedImageData,0,SnapImage.m_InfoHeader.biSizeImage);
	...
	memset(g_ImageBuffer1,0,UHD_IMAGE_SIZE);
	...
	for(int nRow = nStartYposition; nRow <= nEndYposition; nRow++)
	{
		...
		{ 
			...
			//	pProcessedImageData[nRefImageIndex+2] = abs(nDifferenceR);
				...
				if((abs(nDifferenceR) <= nUpperLimit) && (abs(nDifferenceG) >= nLowerLimit)  // R
...
//	int nNoLevel	 = (int)pow(2,(nColorDepth/3));
	...
	{
		DWORD dwFuncElapsedTime = GetTickCount()-dwFuncTickCount;
		szFuncElapsedTime.Format("     ZoomOut [%dms]",dwFuncElapsedTime);
		AddTimeData(szFuncElapsedTime);
	}
...
//	BinaryForDiffImage(NORMAL_BINARY,SnapImage,RefImage,nLowerLimit,nUpperLimit);
	...
	*/
	...
	if(CurrentSet->bSaveProcessingTimeData)
	{
		...
		szFuncElapsedTime.Format("     NoiseTest Loop [%dms]",dwFuncElapsedTime);
		AddTimeData(szFuncElapsedTime);
	}
...
int	CImageProc::MakeAvgImg(CStringArray& aFilePathArray,CBaseImage& DstImage,CProgressCtrl& ItemProgress,CProgressCtrl& TotalProgress)
{
	...
	for(int i=0; i<nNoImage; i++)
	{
		...
		if(!m_aImageList[i].LoadImage(szFilePath,ItemProgress))
		{
			for(int j=0; j<i; j++)
			{
...
BOOL CImageProc::MakeMaskImg(int nMaskWidth,CBaseImage& SrcImage,CBaseImage& DstImage,CProgressCtrl& Progress,BOOL bInvalidateOsdRegion)
{
	...
	nMaskWidth += 14;	//godtech 190608 nMaskWidth += 7;
#endif
	// Dimension of the Snap Image
...
void CImageProc::SetAllowedErrorPixel(double fAllowedErrorPixelPercentage[])
{
	for(int i=0; i<5; i++)
	{
...
void CImageProc::Rotate(CBaseImage& SrcImage, float fRotateFactor)
{
	...
	int nNewHeight = nHeight;// (int)(nHeight * fZoomFactor);
	int nNewWidth = nWidth;//(int)(nWidth  * fZoomFactor);
	...
	for (int r = 0; r < nNewHeight; r++)
	{
		lCopyPoint_H = (nNewHeight - r-1);
		if ((lCopyPoint_H < 0) || (lCopyPoint_H >= nNewHeight))
		{
			continue;
		}
		...
		for (int c = 0; c < nWidth; c++)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				pImageData[lCopyPoint_H *nNewRowWidth + (nWidth - c-1) * 3+i] = SrcImage.m_pImageData[r*nRowWidth + c * 3+i];
			}
			//for (int i = 0; i < 3; i++)
			//{
			//	fRow = r / fZoomFactor;
...
void CImageProc::RedBlueSwap(unsigned char *pImgBufIn, int nWidth, int nHeight, int nRB_Swap)
{
	...
	for (r = 0; r < nHeight; r++)
	{
		...
		for (int c = 0; c < nWidth; c++)
		{
			pImageData[(r*nWidth + c) * 3 + 0] = pImgBufIn[(r*nWidth + c) * 3 + 2];
			pImageData[(r*nWidth + c) * 3 + 1] = pImgBufIn[(r*nWidth + c) * 3 + 1];
			pImageData[(r*nWidth + c)* 3 + 2] = pImgBufIn[(r*nWidth + c) * 3 + 0];
			
			
		}
	}

	memcpy(pImgBufIn, pImageData, nNewImageSize);
	if (pImageData != NULL) delete pImageData; pImageData = NULL;
...
void CImageProc::HsyncModeChange(int nDeHsyncEnable,  int nHyncEdgeEnable)
{

	//if( (nDeHsyncEnable == 0)&&(nHyncEdgeEnable == 0))
	//	return;
	//
	...
	if (nDeHsyncEnable == 0)
		lvds_de_to_hsync = 0;
	if (nHyncEdgeEnable == 0)
		lvds_hsync_edge_en = 0;
	
	__int64 Rdata = 0;
	__int64 Wdata = 0;
	g_pView->m_clsPCI.CM_RegisterRead(0x0160, &Rdata); 
	Sleep(1);
	Wdata = { Rdata | lvds_de_to_hsync | lvds_hsync_edge_en };
	g_pView->m_clsPCI.CM_RegisterWrite(0x0160, Wdata);
	Sleep(1);	
}

void CImageProc::P60_CEDS20Y_Mapping(unsigned char *pImgBufIn, unsigned char *pImgBufOut,  int nWidth, int nHeight)
{
	...
	for ( i = 0; i < nHeight / 2; i = i + 1) {
		for ( j = 0; j < ((nWidth * 3 / 4) * 3 / 18); j = j + 1) {
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 0] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 3];		//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 1] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 0];		//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 2] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 4];		//R
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 3] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 1];		//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 4] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 5];		//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 5] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 2];		//R
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 6] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 11];	//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 7] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 8];		//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 8] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 9];		//R
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 9] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 6];		//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 10] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 10];	//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 11] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 7];		//R
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 12] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 16];	//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 13] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 13];	//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 14] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 17];	//R
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 15] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 14];	//B
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 16] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 15];	//G
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 17] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 12];	//R

			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 0] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 4];		//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 1] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 1];		//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 2] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 5];		//R
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 3] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 2];		//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 4] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 3];		//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 5] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 0];		//R
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 6] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 9];		//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 7] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 6];		//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 8] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 10];	//R
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 9] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 7];		//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 10] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 11];	//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 11] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 8];		//R
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 12] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 17];	//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 13] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 14];	//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 14] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 15];	//R
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 15] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 12];	//B
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 16] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 16];	//G
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 17] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 13];	//R
		}
	}

	memcpy(pImgBufOut, pImageData, nNewImageSize);
	if (pImageData != NULL) delete pImageData; pImageData = NULL;

	
}

void CImageProc::SW_MODE_Mapping(unsigned char *pImgBufIn, unsigned char *pImgBufOut,  int nWidth, int nHeight, int nSW_Mode)
{
	//g_ImageProc.P60_CEDS20Y_Mapping(bufTmp, g_GrabDisplayImage.m_pImageData, nWidth, nHeight);
	if (nSW_Mode != 3)
		return;
...
//	int nProgress = 0;
	...
	//21.02.04 Boe 3.0 신규 60핀
	for ( i = 0; i < nHeight / 2; i = i + 1) {
		for ( j = 0; j < ((nWidth * 3 / 4) * 3 / 18); j = j + 1) {
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 0] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 1];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 1] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 0];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 2] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 2];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 3] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 13];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 4] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 12];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 5] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 14];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 6] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 11];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 7] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 10];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 8] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 9];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 9] =  pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 7];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 10] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 6];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 11] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 8];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 12] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 5];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 13] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 4];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 14] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 3];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 15] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 17];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 16] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 16];
			pImageData[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 17] = pImgBufIn[(nHeight - i * 2 - 1) * nWidth * 3 + j * 18 + 15];

			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 0] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 0];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 1] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 1];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 2] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 2];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 3] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 12];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 4] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 13];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 5] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 14];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 6] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 9];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 7] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 10];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 8] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 +  11];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 9] =  pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 6];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 10] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 7];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 11] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 8];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 12] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 3];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 13] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 4];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 14] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 5];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 15] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 15];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 16] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 16];
			pImageData[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 17] = pImgBufIn[(nHeight - i * 2 - 2) * nWidth * 3 + j * 18 + 17];
		}
	}
...
void CImageProc::P68_50_UPQO_VA_CEDS_HKC(unsigned char *bufTmp, unsigned char *pImgBufOut,  int nWidth, int nHeight)
{
	//g_ImageProc.P60_CEDS20Y_Mapping(bufTmp, g_GrabDisplayImage.m_pImageData, nWidth, nHeight);
	...
	//20.09.16 신규 1.5G 68핀 50 UPQO VA CEDS #338 HKC 일본향 ARIB
	for (i = 0; i < nHeight / 2; i = i + 1) {
		for (j = 0; j < nWidth; j = j + 1) {
			//Odd
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0]; //B
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 1] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 2]; //G
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 2] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 1]; //R
			//Even
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 2]; //B
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 1] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 1]; //G
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 2] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 0]; //R
		}
	}
...
void CImageProc::P68_55UP77_CSOT(unsigned char *bufTmp, unsigned char *pImgBufOut,  int nWidth, int nHeight)
{
	//g_ImageProc.P60_CEDS20Y_Mapping(bufTmp, g_GrabDisplayImage.m_pImageData, nWidth, nHeight);
	...
	//20.09.16 신규 1.5G 68핀 50 55UP77 CSOT
	for (i = 0; i < nHeight / 2; i = i + 1) {
		for (j = 0; j < nWidth; j = j + 1) {
			//Odd
			//bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0]; //B
			...
			//Odd
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 1]; //B
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 1] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0]; //G
			bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 2] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 2]; //R
			//Even
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 0]; //B
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 1] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 2]; //G
			bufOut[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 2] = bufTmp[(nHeight - i * 2 - 2) * nWidth * 3 + j * 3 + 1]; //R
...
void CImageProc::P68_70_UPQO_Sharp_CEDS_DPT(unsigned char *bufTmp, unsigned char *pImgBufOut, int nWidth, int nHeight)
{
	//g_ImageProc.P60_CEDS20Y_Mapping(bufTmp, g_GrabDisplayImage.m_pImageData, nWidth, nHeight);
...
//20.09.16 신규 68핀 1.5G 70 UPQO Sharp CEDS DPT #331
for (i = 0; i < nHeight / 2; i = i + 1) {
	for (j = 0; j < nWidth; j = j + 1) {
		//Odd
		bufOut[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 0] = bufTmp[(nHeight - i * 2 - 1) * nWidth * 3 + j * 3 + 1]; //B
...
void CImageProc::P22Y8K_TCONLESS(unsigned char *bufTmp, unsigned char *pImgBufOut, int nWidth, int nHeight)
{
	//g_ImageProc.P60_CEDS20Y_Mapping(bufTmp, g_GrabDisplayImage.m_pImageData, nWidth, nHeight);
	...
	for (i = 0; i < nHeight / 2; i = i + 1) {
		...
		{
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 1) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 3) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 5) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 0) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 7) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 2) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 4) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 9) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 6) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 2];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 1] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 0];
			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 2] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 8) * 3 + 1];

			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 0] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 0];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 1] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 1];
			bufOut[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 2] = bufTmp[(i * 2 + 0) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 2];

			bufOut[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 11) * 3 + 0] = bufTmp[(i * 2 + 1) * nWidth * 3 + (j + (nWidth / 16) * 10) * 3 + 2];
...
int	CImageProc::DFT3_UHDPuzzleLocal(int nMode, unsigned char *bufIn, unsigned char *bufOut, int nWidth, int nHeight, int nRotate, int nModel)
{
	...
	case 2:			//	150224 - cks 4 division 대응
		if (nRotate == 0) {
			...
			if (nModel == PACK_TYPE_STANDBYME) {
				for (i = 0; i < nHeight; i = i + 1) {

					for (j = 0; j < nWidth / 8; j = j + 1) {
						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];

						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
					}

					for (j = 0; j < nWidth / 8; j = j + 1) {

						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 2];

						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2 + 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 2];

					}
					for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];

						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2 - 640 * 3] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
					}

					for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + 2 * 3 + 2];

						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + 3 * 3 + 2];
				...
				for (i = 0; i < nHeight; i = i + 1) {
					for (j = 0; j < nWidth / 8; j = j + 1) {
						...
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];

						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
					...
					for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
						...
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
						...
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[i*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[i*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
						bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[i*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
		...
		if (nRotate == 0) {
			...
			if (nModel == PACK_TYPE_STANDBYME) {
				...
				for (i = 0; i < nHeight; i = i + 1) {
					if (i % 2 == 0) {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[i / 2 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
					else {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 2 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
				}
			}
		...
		if (nRotate == 0) {
			...
			if (nModel == PACK_TYPE_STANDBYME) {
				...
				for (i = 0; i < nHeight; i = i + 1) {
					if (i % 4 == 0) {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[i / 4 * nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
					else if (i % 4 == 1) {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[(540 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
					else if (i % 4 == 2) {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[(1080 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
					else if (i % 4 == 3) {
						for (j = 0; j < nWidth / 8; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 0 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + 6 * j + 1 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
						for (j = nWidth / 8; j < nWidth / 4; j = j + 1) {
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 0 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 0 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 4 + 6 * j + 1 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + 1 * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (0 + 2) * 3 + 2];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 0] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 0];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 0 * 3 + 1] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 1];
							bufOut[(nHeight - 1 - i)*nWidth * 3 + nWidth * 3 / 2 + 6 * j + 1 * 3 + 2] = bufIn[(1620 + (i / 4 + 1))*nWidth * 3 + j * 3 * 4 + (1 + 2) * 3 + 2];
						}
					}
				}
			}
