...
void CParmAdjPage::OnBtnParmOpenGrabImg() 
{
	...
	"Grab Image Files(*.bmp) | *.bmp|Grab Image Files(*.jpg) | *.jpg|",NULL);
...
//		sTmp = CurrentSet->sModelFolder;
	...
	if(dlg.DoModal() == IDOK) 
	{
		...
		szFileExt = dlg.GetFileExt();
		if(szFileExt == "bmp"){
			OpenImgFile(m_strGrabPath, I_GRAB, TRUE);
			OpenImgFile(m_strGrabPath, I_PROC, TRUE);
		}
		else{
			OpenImgFile(m_strGrabPath, I_GRAB, FALSE);
			OpenImgFile(m_strGrabPath, I_PROC, FALSE);
		}
		*m_nImgType = I_GRAB;
	}
}
...
BOOL CParmAdjPage::OpenImgFile(CString sImgPath, int nImgType, BOOL bBmp)
{
	CString sTmp;
	...
	if(bBmp){
		...
		if(!m_Image[nImgType].LoadJpgImage(sImgPath, *m_ctrlProgress)) return FALSE;
	}
...
#ifdef SM_CYS_TEMP_TEST_CODE__  //SM_MODIFY_CODE__
...
//	CString sTmp = _T("");
//	GetDlgItemText(IDC_EDIT_PARM_ROI_WIDTH, sTmp);
	nMaskWidth =  m_ctrlMaskWidthCmb.GetCurSel() +1;

	//================
	// Skip GrabNLoad
	//================
	szFileName =  "D:\\DFT\\Ref_18\\SIMPLE TEST\\SIMPLE TEST\\Ref\\AV1.bmp" ;
	...
	CBaseImage l_BaseImage;
	CBaseImage l_MaskImage;

	l_BaseImage.LoadImage(szFileName, *m_ctrlProgress);
	...
	for (int i = 0; i < 2; i++)
	{
		//+change kwmoon 080820
	//	g_ImageProc.Average(nXMask, nYMask, g_pView->m_Image[I_AVG], TRUE);
		g_ImageProc.Average(nXMask, nYMask, l_BaseImage);
	}
		

		
	if (!g_ImageProc.MakeMaskImg(nMaskWidth, l_BaseImage, l_MaskImage, *m_ctrlProgress, FALSE))
	{
	...
	// Save Mask Image
	//=================
	//szMaskFilePath = g_pView->m_szMakeRefPath + "\\Mask\\";D:\DFT\Ref_18\SIMPLE TEST\SIMPLE TEST
	szMaskFileName.Format(_T("D:\\DFT\\Ref_18\\SIMPLE TEST\\SIMPLE TEST\\Ref\\Mask\\SM1_W%d.bmp"), nMaskWidth);
	...
	l_MaskImage.SaveImage(szMaskFileName, MASK_IMAGE, *m_ctrlProgress);
...
//	ZeroMemory(szInitFileName,100);
	...
	CString szFileName;
	CString szFileExt;
	CString szFilter = "BMP (*.bmp) | *.bmp|JPG (*.jpg) | *.jpg||";
...
//		szFileName = dlg.GetFileName();
		...
		if(szFileExt == "bmp"){
			...
			if(!pBaseImage->SaveJpgImage(szFileName,*m_nImgType,*m_ctrlProgress)){ return;}
		}
