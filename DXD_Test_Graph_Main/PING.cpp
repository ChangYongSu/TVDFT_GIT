...
int CPing::Ping(UINT nRetries,LPCSTR pstrHost, UINT nWaitMS)
{
	SOCKET	  rawSocket;
	LPHOSTENT lpHost;
	UINT	  nLoop;
...
//		::PostMessage(m_hWnd,WM_MSG_STATUS, 0, (LPARAM)str.GetBuffer());
//		::PostMessage(m_hWnd,WM_PING_END, 0, 0);
	...
	//			REQ_DATASIZE);
	//::PostMessage(m_hWnd,WM_MSG_STATUS, 1, (LPARAM)str.GetBuffer());
	...
	for (nLoop = 0; nLoop < nRetries; nLoop++)
	{
		...
		//	str.Format("Request Timed Out");
			lError = 1;//::PostMessage(m_hWnd,WM_MSG_STATUS, 3, (LPARAM)str.GetBuffer());
			break;
		}
		else
		...
		//	::PostMessage(m_hWnd,WM_MSG_STATUS, 2, (LPARAM)str.GetBuffer());
			lError = 0;
			Sleep(nWaitMS);
		}
	}
...
//	::PostMessage(m_hWnd,WM_MSG_STATUS, 0, (LPARAM)strMsg.GetBuffer());
}
