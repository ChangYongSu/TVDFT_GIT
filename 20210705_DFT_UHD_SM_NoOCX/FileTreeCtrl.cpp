...
CSystemImageList::CSystemImageList()
{
  ...
  if (m_nRefCount == 0)
  {
	...
	m_ImageList.SetBkColor(RGB(238,239,231));
  }  
...
BOOL CTreeFileCtrl::Delete(HTREEITEM hItem)
{
  ...
  if (!IsDrive(hItem) && m_bAllowDelete)
  {
    ...
    _tcscpy_s(pszCur, sizeof(pszCur),  sFileToDelete);
...
//DEL void CTreeFileCtrl::OnFileRename() 
//DEL {
//DEL   Rename(GetSelectedItem());
//DEL }


//DEL void CTreeFileCtrl::OnFileOpen() 
//DEL {
//DEL   Open(GetSelectedItem());
//DEL }

//DEL void CTreeFileCtrl::OnFileDelete() 
//DEL {
//DEL   Delete(GetSelectedItem());
//DEL }

//DEL void CTreeFileCtrl::OnUpdateFileDelete(CCmdUI* pCmdUI) 
//DEL {
//DEL 	pCmdUI->Enable(!IsDrive(GetSelectedItem()) && m_bAllowDelete);
//DEL }
...
//DEL void CTreeFileCtrl::OnViewRefresh() 
//DEL {
//DEL   SetRedraw(FALSE);
//DEL 
//DEL   //Get the item which is currently selected
//DEL   HTREEITEM hSelItem = GetSelectedItem();
//DEL   CString sItem  = ItemToPath(hSelItem);
//DEL   BOOL bExpanded = (GetChildItem(hSelItem) != NULL); 
//DEL 
//DEL   //Display the folder items in the tree
//DEL   if (m_sRootFolder.IsEmpty())
//DEL     DisplayDrives(TVI_ROOT, FALSE);
//DEL   else
//DEL     DisplayPath(m_sRootFolder, TVI_ROOT, FALSE);
//DEL   
//DEL   //Reselect the initially selected item
//DEL   if (sItem.GetLength())
//DEL     hSelItem = SetSelectedPath(sItem, bExpanded);
//DEL 
//DEL   SetRedraw(TRUE);
//DEL }
...
void CTreeFileCtrl::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  if (pDispInfo->item.pszText)
  {
    ...
    _tcscpy_s(pszFrom, sizeof(pszFrom), sFrom);
    ...
    _tcscpy_s(pszTo, sizeof(pszTo), sTo);
...
void CTreeFileCtrl::DisplayDrives(HTREEITEM hParent, BOOL bUseSetRedraw)
{
  ...
  for (int i=0; i<32; i++)
  {
    if (dwDrives & dwMask)
    {
	  ...
	  if(sDrive != "A:\\") InsertFileItem(sDrive, sDrive, hParent);
    }
    dwMask <<= 1;
  }
...
void CTreeFileCtrl::DisplayPath(const CString& sPath, HTREEITEM hParent, BOOL bUseSetRedraw)
{
  ...
  for (i=0; i<DirectoryPaths.GetSize(); i++)
  {
    CString& sPath = DirectoryPaths.ElementAt(i);
    TCHAR path_buffer[_MAX_PATH];
    TCHAR fname[_MAX_FNAME];
    TCHAR ext[_MAX_EXT];
    _tsplitpath_s(sPath, NULL, _MAX_DRIVE, NULL, _MAX_DIR, fname, _MAX_FNAME, ext, _MAX_EXT);
 
	_tmakepath_s(path_buffer, sizeof(path_buffer), NULL, NULL, fname, ext);
...
void CTreeFileCtrl::EndDragging(BOOL bCancel)
{
  if (IsDragging())
  {
    ...
    if (!bCancel)
    {
      ...
      TCHAR* pszFrom = new TCHAR[nFromLength + 2];
      _tcscpy_s(pszFrom, sizeof(pszFrom), sFromPath);
      pszFrom[nFromLength+1] = _T('\0');
      shfo.pFrom = pszFrom;

      TCHAR* pszTo = new TCHAR[nToLength + 2];
      _tcscpy_s(pszTo, sizeof(pszTo), sToPath);
      pszTo[nToLength+1] = _T('\0');
      shfo.pTo = pszTo;
...
void CTreeFileCtrl::ViewRefresh()
{
	   SetRedraw(FALSE);
