...
void CDigistring::Format(LPCTSTR lpszFormat, ...)
{
	...
	_vsntprintf_s(szMessage, sizeof(szMessage),255, lpszFormat, argp);
	va_end(argp);                               
...
void CDigistring::SetTextAlign(DWORD dwAlignStyle)
{
	if(m_AlignStyle == dwAlignStyle) return;

	m_AlignStyle = dwAlignStyle;
...
//	rect.bottom  = rect.bottom -2;
	...
	if (m_DispStyle & DS_SZ_PROP)
	{
		...
		if(m_AlignStyle == AL_LEFT)   	
			CharRect.left = rect.left + 2 ;
		else if(m_AlignStyle == AL_RIGHT)
...
//	::DeleteObject((HPEN)OffPen);
//	::DeleteObject((HPEN)OnPen);
//	::DeleteObject((HBRUSH)hBrushOff);
//	::DeleteObject((HBRUSH)hBrushOn); 
}
