...
void CStopProcessingDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_STATIC_SYSTEM_MESSAGE, m_szSystemMessage);
	//}}AFX_DATA_MAP
}
...
void CStopProcessingDlg::SetMessage(int nMessageType)
{
	...
	switch(nMessageType)
	{
		...
		break;
		case TOOL_OPTION_DOWNLOAD_MSG :
		{
			szMessage.Format("Please wait.. Tool Option Downloading.");
		//	m_ctrlMessage.SetCaption((LPCTSTR)szMessage); 
		//	m_ctrlMessage.SetCaption("Please wait for finishing grab stop process"); 
		}
		break;
	}
	m_szSystemMessage = szMessage;
	UpdateData(FALSE);
}

void CStopProcessingDlg::OnTimer(UINT nIDEvent) 
{
