...
//	char *sHeader[] = {"NO","STEP NAME", "STEP TYPE", "TEST", "SOURCE", "TARGET", "L-LIMIT", "U-LIMIT", "UNIT", "A/T(FREQ,LVL)", "A/M(FREQ,LVL)", "A-MARGIN"};
	char *sHeader[] = {"NO","SOURCE","STEP NAME", "AUDIO","STEP TYPE", "TEST", "TARGET", "L-LIMIT", "U-LIMIT", "UNIT", "A-SOURCE", "AUD(FREQ,LVL)","A-MARGIN"};
//	char *sHeader[] = {"NO","STEP NAME", "STEP TYPE", "TEST", "SOURCE", "TARGET", "L-LIMIT", "U-LIMIT", "UNIT"};
	//-

	m_ctrlMainGrid.SetRedraw(FALSE);
...
void CSeqEditorDlg::UpdateMainGrid()
{
	...
	if(StepList.GetCount() > 0)
	{
		...
		while (Position != NULL) 
		{
			...
			m_ctrlMainGrid.SetCellAlignment(flexAlignLeftCenter);
			m_ctrlMainGrid.SetTextMatrix(nTmp,2,pStep->GetStepName());
			...
			{
				m_ctrlMainGrid.SetTextMatrix(nTmp,3,"O");
			}
			else
			{
				m_ctrlMainGrid.SetTextMatrix(nTmp,3,"X");
			}
			...
			m_ctrlMainGrid.SetCellAlignment(flexAlignLeftCenter);
			m_ctrlMainGrid.SetTextMatrix(nTmp,4,sTmp);
			...
			if(pStep->m_nStepType == CHECK_FUNCTION)
			{
				...
				m_ctrlMainGrid.SetCellAlignment(flexAlignLeftCenter);
				m_ctrlMainGrid.SetTextMatrix(nTmp ,5,pStep->m_strFuncName);
				...
				m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
				m_ctrlMainGrid.SetTextMatrix(nTmp,6,sTmp);
				...
				if(pStep->GetFuncType() != MEAS_BOOL)
				{
					...
					m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
					m_ctrlMainGrid.SetTextMatrix(nTmp,7,sTmp);
					...
					m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
					m_ctrlMainGrid.SetTextMatrix(nTmp,8,sTmp);
					...
					m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
					m_ctrlMainGrid.SetTextMatrix(nTmp,9,sTmp);
				}
				...
				if(pStep->m_bRunAudioTest)
				{
					...
					m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
					m_ctrlMainGrid.SetTextMatrix(nTmp,10,sTmp);
					...
					if(pStep->m_nMeasureAudioType == LEVEL_CHECK) {
						...
						m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
						m_ctrlMainGrid.SetTextMatrix(nTmp,11,sTmp);
					}
						...
						m_ctrlMainGrid.SetCellAlignment(flexAlignCenterCenter);
						m_ctrlMainGrid.SetTextMatrix(nTmp,12,sTmp);
					}
				}
...
void CSeqEditorDlg::InitSubGrid()
{
	...
	char *sHeader[] = {"STEP NAME", "DATA1", "DATA2", "DATA3", "DATA4", "DATA5", "DATA6", "DATA7", "DATA8", "DATA9", "DATA10"};

	m_ctrlSubGrid.SetRedraw(FALSE);
					...
					if(sTmp.Compare("send_i2c_command") == NULL)
					{
						if(nArgumentNo == 2)
						{
							switch(pArgument->Integer)
							{
								...
								case 116 : sData1 = "I2C_CHK_MAXMIN2";	break;
								case 117 : sData1 = "I2C_CHK_LIMIT2";	break;
						}
...
void CSeqEditorDlg::SetSubGrid4Edit(int nFuncNo, int nRow, int nCol)
{
	...
	//+ 2007.10.11 Add BY USY
	if(Predefined[nFuncNo].argType[nCol - 1] != NULL_TYPE)
	{
...
/*				CFileDialog dlg(TRUE, "bmp", NULL, 
				...
				CString sTmp;
				dlg.m_ofn.lpstrInitialDir = CurrentSet->sModelFolder;

				if(dlg.DoModal() == IDOK)
				{
					sTmp.Format("\"%s\"", dlg.GetFileName());
...
//				dlg.m_strModelFolder = CurrentSet->sModelFolder + "\\ref\\";
				...
				dlg.m_nPrevRoiX1 = atoi(m_ctrlSubGrid.GetTextMatrix(nRow, 1));
				dlg.m_nPrevRoiY1 = atoi(m_ctrlSubGrid.GetTextMatrix(nRow, 2));
				dlg.m_nPrevRoiX2 = atoi(m_ctrlSubGrid.GetTextMatrix(nRow, 3));
				dlg.m_nPrevRoiY2 = atoi(m_ctrlSubGrid.GetTextMatrix(nRow, 4));
				...
				if(dlg.DoModal() == IDOK)
				{
					...
					m_ctrlSubGrid.SetTextMatrix(nRow, 1, sStartX);
					m_ctrlSubGrid.SetTextMatrix(nRow, 2, sStartY);
					m_ctrlSubGrid.SetTextMatrix(nRow, 3, sEndX);
					m_ctrlSubGrid.SetTextMatrix(nRow, 4, sEndY);
				}			
			}
			break;
...
//				sList.Format("|RF|AV|AV_YC|AV_1|AV1_YC1|AV2|AV2_YC2|AV3|AV3_YC3|COMP|COMP1|COMP2|RGB|HDMI|HDMI1|HDMI2|HDMI3");
		...
		case AVC_INPUT:
			{
				CString sTmp;
				sTmp.Format("|0\tRGB|1\tDVI|2\tDP|3\tHDMI");	
				m_ctrlSubGrid.SetRow(nRow);
				m_ctrlSubGrid.SetEditable(flexEDKbdMouse);
				m_ctrlSubGrid.SetColComboList(nCol, sTmp);
		...
		case HDMI_GRAB_RESOLUTION:
			{
				CString sTmp;
				sTmp.Format("|0\t1028 x 768(XGA)|1\t1366 x 768(HD)|2\t1280 x 1024(SXGA)|3\t1920 x 1080(FHD)");	
				m_ctrlSubGrid.SetRow(nRow);
				m_ctrlSubGrid.SetEditable(flexEDKbdMouse);
				m_ctrlSubGrid.SetColComboList(nCol, sTmp);
		...
		case HDMI_GRAB_BITSHIFT:
			{
				CString sTmp;
				sTmp.Format("|0\t0Bit Shift(Data[7:0])|1\t1Bit Shift(Data[8:1])|2\t2Bit Shift(Data[9:2])|3\t3Bit Shift(Data[10:3])|4\t4Bit Shift(Data[11:4])");	
				m_ctrlSubGrid.SetRow(nRow);
				m_ctrlSubGrid.SetEditable(flexEDKbdMouse);
				m_ctrlSubGrid.SetColComboList(nCol, sTmp);
...
CString CSeqEditorDlg::MakeMainStepComboList(int nType)
{
	...
	switch(nType) 
	{
		...
		case CURSRC_CMB:
			if(g_nGrabberType == FHD_GRABBER)
			{
				for(nTmp = 0; InputDefined_FHD[nTmp].pszName != NULL; nTmp++)
				{
					sTmp.Format("|%s", InputDefined_FHD[nTmp].pszName);
					sList = sList + sTmp;
				}
			}
			else{
				for(nTmp = 0; InputDefined_UHD[nTmp].pszName != NULL; nTmp++)
				{
					sTmp.Format("|%s", InputDefined_UHD[nTmp].pszName);
					sList = sList + sTmp;
				}
			}
...
//	sTmp = m_ctrlMainGrid.GetTextMatrix(nRow, 4);
	sTmp = m_ctrlMainGrid.GetTextMatrix(nRow, MAIN_STEP_TYPE_COL);
...
//	sTmp = m_ctrlMainGrid.GetTextMatrix(nRow, 5);
	sTmp = m_ctrlMainGrid.GetTextMatrix(nRow, MAIN_TEST_COL);
	sTmp.MakeLower();
	sTmp.TrimLeft();
	sTmp.TrimRight();
...
//		for(nTmp = 0; Predefined[nTmp].pszFunc != TNULL; nTmp++)
		for(int nTmp = 0; Predefined[nTmp].pszFunc != NULL; nTmp++)
		{
		...
		for(int nTmp =0; Predefined[nSelNo].achPara[nTmp] != TNULL; nTmp++)
		{
			...
			if(Predefined[nSelNo].achPara[nTmp] == INTEGER)
			{
				...
				if(sSubStep.Compare("send_i2c_command") == NULL)
				{
					if(nArgumentNo == 2)
					{
						...
						else if(sTmp.Compare("i2c_chk_maxmin2") == NULL) pArgument->Integer = 116;
						else if(sTmp.Compare("i2c_chk_limit2")  == NULL) pArgument->Integer = 117;
						else
						{
			...
			else if((Predefined[nSelNo].achPara[nTmp] == STRING))
			{
				...
				strcpy_s(pArgument->String, _countof(pArgument->String), sTmp);
				pFunc->m_ArgumentList.AddTail(pArgument);
				nCol++;
			}
			//+ 2007.10.10 Add BY USY
...
void CSeqEditorDlg::OnBtnInsertFromFullseq() 
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		POSITION OrgPos = FullSeq.FindIndex(dlg.m_nSelRow - 1);
...
void CSeqEditorDlg::InitTemplateGrid()
{
	...
	m_ctrlTemplateGrid.SetRedraw(FALSE);
...
//	m_ctrlTemplateGrid.SetAllowSelection(FALSE);
	...
	m_ctrlTemplateGrid.SetRedraw(TRUE);
...
void CSeqEditorDlg::UpdateTemplateGrid()
{
	...
	if(FullSeq.GetCount() > 0)
	{
		...
		while (Position != NULL) 
		{
			...
			if(pStep->m_bRunAudioTest)
			{
				m_ctrlTemplateGrid.SetTextMatrix(nTmp,4,"O");
			}
			else
			{
				m_ctrlTemplateGrid.SetTextMatrix(nTmp,4,"X");
			}
...
{
	// TODO: Add your control notification handler code here
	m_nTemplateSelectedRow = m_ctrlTemplateGrid.GetRow();
