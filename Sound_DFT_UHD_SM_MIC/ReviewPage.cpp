...
//	char *sHeader[] = {"NO","STEP", "RESULT", "MEAS", "TARGET", "L-LIMIT", "U-LIMIT", "UNIT", "TIME(s)", "MESSAGE"};
	//-

	m_ctrlReviewGrid.SetRedraw(FALSE);
...
//	m_ctrlReviewGrid.SetRows(11);
	...
	{
		m_ctrlReviewGrid.SetCol(nTmp);
		//m_ctrlReviewGrid.SetCellAlignment(4);
		m_ctrlReviewGrid.SetFillStyle(flexFillRepeat);
		m_ctrlReviewGrid.SetFontName("Arial");
		m_ctrlReviewGrid.SetFontSize(10);
		m_ctrlReviewGrid.SetExtendLastCol(TRUE);
		m_ctrlReviewGrid.SetText(sHeader[nTmp]);
		
		switch(nTmp) 
		{
			case 0:
				{	// Col 0 : NO
					m_ctrlReviewGrid.SetColWidth(nTmp, 500);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 1:
				{	// Col 1 : STEP
					m_ctrlReviewGrid.SetColWidth(nTmp, 3700);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 3:
				{	// Col 3 : MEAS
					m_ctrlReviewGrid.SetColWidth(nTmp, 1300);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 2:		case 4: 		case 5:		case 6:
				{	// Col 2 : RESULT // Col 4 : TARGET // Col 5 : L-LIMIT // Col 6 : U-LIMIT
					m_ctrlReviewGrid.SetColWidth(nTmp, 1000);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 7:
				{	// Col 7 : UNIT
					m_ctrlReviewGrid.SetColWidth(nTmp, 800);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 8:
				{	// Col 8 : Audio Target
					m_ctrlReviewGrid.SetColWidth(nTmp, 1900);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			...
			case 10:
				{	// Col 10 : Audio Margin
					m_ctrlReviewGrid.SetColWidth(nTmp, 1200);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 11:
				{	// Col 11 : TIME(s)
					m_ctrlReviewGrid.SetColWidth(nTmp, 900);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
			case 12:
				{	// Col 12 : MESSAGE
					m_ctrlReviewGrid.SetColWidth(nTmp, 5000);
					m_ctrlReviewGrid.SetColDataType(nTmp, flexDTString);
					m_ctrlReviewGrid.SetColAlignment(nTmp, flexAlignCenterCenter);
				}
				break;
	...
	m_ctrlReviewGrid.SetRedraw(TRUE);
...
void CReviewPage::UpdateGrid()
{
	...
	if(m_StepList.GetCount() > 0)
	{
		...
		while (Position) 
		{
			...
			m_ctrlReviewGrid.SetTextMatrix(nTmp , 0, sTmp);
		
			//Step Name
			m_ctrlReviewGrid.SetTextMatrix(nTmp , 1, pStep->GetStepName());

		    if(pStep->m_nStepType == PROCESS_FUNCTION)
			{
				//ElapsedTime
				sTmp.Format(_T("%5.1f"), pStep->m_fElapsedTime);
				m_ctrlReviewGrid.SetTextMatrix(nTmp, 11, sTmp);
				m_ctrlReviewGrid.SetRedraw(TRUE);
				m_ctrlReviewGrid.Refresh();
			...
			{	//Result
				if(pStep->m_bTest == TRUE)
				{
					if (pStep->m_bResult)
					{
						...
						m_ctrlReviewGrid.SetCellBackColor(RGB(0,128,64));
						m_ctrlReviewGrid.SetCellForeColor(RGB(255,255,255));
						m_ctrlReviewGrid.SetTextMatrix(nTmp, 2, sTmp);
				...
				m_ctrlReviewGrid.SetTextMatrix(nTmp , 3, sTmp);
				...
				m_ctrlReviewGrid.SetTextMatrix(nTmp , 4, sTmp);
...
//					m_ctrlReviewGrid.SetTextMatrix(nTmp , 5, sTmp);
					...
					m_ctrlReviewGrid.SetTextMatrix(nTmp , 6, sTmp);
					//Unit
					sTmp.Format(_T("%s"), pStep->GetUnit());
					m_ctrlReviewGrid.SetTextMatrix(nTmp ,7, sTmp);
				}
				//Time
				sTmp.Format(_T("%5.1f"), pStep->m_fElapsedTime);
				m_ctrlReviewGrid.SetTextMatrix(nTmp, 11, sTmp);
				...
				//Message
				m_ctrlReviewGrid.SetTextMatrix(nTmp, 12, pStep->m_strMsg);
				//-
				...
				if(pStep->m_bRunAudioTest)
				{
					...
					m_ctrlReviewGrid.SetTextMatrix(nTmp , 8, sTmp);
					...
					m_ctrlReviewGrid.SetTextMatrix(nTmp , 9, sTmp);
					
					// (Column 10) Audio Margin
					sTmp.Format(_T("%d"), pStep->m_nAudioMargin);
					m_ctrlReviewGrid.SetTextMatrix(nTmp, 10, sTmp);
				}
				//-
			}
