...
void CSeqRoiDlg::DrawImage(int nDlgItemNo,CBaseImage& ImageData, int nStartXPos, int nStartYPos)
{
	...
	if((nHeight < rect.Height()) && (nWidth  < rect.Width()))
	{
		...
		if((m_nPrevRoiX1 < (UINT)(m_nImageWidth-1)) 
		&& (m_nPrevRoiX2 < (UINT)(m_nImageWidth-1)) 
		&& (m_nPrevRoiX2 > (UINT)m_nPrevRoiX1)
		&& (m_nPrevRoiY1 < (UINT)(m_nImageHeight-1)) 
		&& (m_nPrevRoiY2 < (UINT)(m_nImageHeight-1)) 
		&& (m_nPrevRoiY2 > (UINT)m_nPrevRoiY1))
		{
...
void CSeqRoiDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if((m_nDisplayType == DISPLAY_TYPE_FULL)
	|| ((m_nImageWidth < m_ImgViewRect.Width()) && (m_nImageHeight < m_ImgViewRect.Height())))
	{
		m_nRoiX1 = nPointX;
		m_nRoiY1 = nPointY;
	}
	else
	{
		...
		if(m_nRoiX1 > (UINT)(m_nImageWidth-1))  m_nRoiX1 = (m_nImageWidth-1);
		if(m_nRoiY1 > (UINT)(m_nImageHeight-1)) m_nRoiY1 = (m_nImageHeight-1);
	}
...
void CSeqRoiDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(m_bFlagMouse == TRUE)
	{
		...
		else
		{
			...
			if(m_nRoiX2 > (UINT)(m_nImageWidth-1))  m_nRoiX2 = (m_nImageWidth-1);
			if(m_nRoiY2 > (UINT)(m_nImageHeight-1)) m_nRoiY2 = (m_nImageHeight-1);
		}
