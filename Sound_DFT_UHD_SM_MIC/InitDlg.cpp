...
/*		int nTmp = 0;
	...
	//	for(int i = 0 ; i < m_saLineList.GetSize() ; i++)
		...
		if(m_saLineList.GetSize() > 0)
		{
			for(int i = 0 ; i < m_saLineList.GetSize() ; i++)
			{
				...
				sBuf2.TrimLeft(' ');
				sBuf2.TrimRight(' ');
				nNoList1 = _ttoi((const TCHAR*)sBuf2);
				if(SplitIt.GetSize() > 3)
				{
					sBuf2 = SplitIt.GetAt(3);
					sBuf2.TrimLeft(' ');
					sBuf2.TrimRight(' ');
					nNoList2 = _ttoi((const TCHAR*)sBuf2);

					sBuf2 = SplitIt.GetAt(4);
					sBuf2.TrimLeft(' ');
					sBuf2.TrimRight(' ');
					nNoList3 = _ttoi((const TCHAR*)sBuf2);
				}

				if(szLineID.Compare(sBuf) == NULL)
				{
					CurrentSet->nLineNo	= nNoList1;
					CurrentSet->nLineID_GLZ = nNoList1;
					CurrentSet->nLineID_GPZ = nNoList2;
					CurrentSet->nLineID_GMZ = nNoList3;
					break;
				}
			}
		}
	...
	g_pView->SetTimer(2, 1000, NULL);
	g_pView->SetTimer(3, 2000, NULL);
	g_pView->SetTimer(TIMER_AUTO_CONTROL_ID, 1000, NULL);//SetTimer(TIMER_AUTO_CONTROL_ID, 500, NULL);
...
//	m_MWnd->m_pCheckDeviceDlg = NULL;
	...
	if(g_nGrabberType == UHD_GRABBER)
	{
		m_MWnd->StopLVDSGrabThread();
	}//
	else//FHD
	{
		m_MWnd->StopLVDSGrabThread_FHD();
	}
